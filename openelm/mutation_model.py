...
def get_model(config: ModelConfig):
    ...
    elif config.model_type == "hf2":
        return HuggingFaceLLM2(config=config)   
    ...
    def _generate(
        ...
        for i in range(total_batches):
            ...
            if self.config.logits_only:
                ...
                tokens = F.softmax(logits, dim=-1).argmax(dim=-1)
                texts: list[str] = self.tokenizer.batch_decode(
                        tokens
                    )
                generations: list[Generation] = [
                    Generation(text=texts, generation_info={"logits": logits})
                ...
                terminators = [
                    ...
                    self.tokenizer.convert_tokens_to_ids("}"), # Interrupt after completing generating a JSON format (no yapping)
...
#                     print(self.tokenizer.batch_decode(tokens[:, input_ids_len:, ...].T))
    ...
    def chatbot_gen(self, prompts: str, stop: Optional[list[str]] = None,  system_prompts: str = None,):
        """Run the LLM on the given prompt and input."""
        if self.tokenizer.pad_token is None:
            self.tokenizer.add_special_tokens({'pad_token': '[PAD]'})
            self.model.resize_token_embeddings(len(self.tokenizer))
    ...
    def gen(
        ...
        pos_list = ['yes', ' yes', 'Yes', ' Yes', 'YES', ' YES']
        neg_list = ['no', ' no', 'No', ' No', 'NO', ' NO']
        pos_encoding, neg_encoding = [], []
        for i in range(len(pos_list)):
            pos_encoding.append(self.tokenizer.encode(pos_list[i])[-1])
            neg_encoding.append(self.tokenizer.encode(neg_list[i])[-1])
        ...
        for i in range(total_batches):
            ...
            for j in range(len(pos_list)):
                pos_prob += softmax[0, -1, int(pos_encoding[j])]
                neg_prob += softmax[0, -1, int(neg_encoding[j])]
            
            if i == 0:
                opinion1_prob += pos_prob
                opinion2_prob += neg_prob
            elif i == 1:
                opinion1_prob += neg_prob
                opinion2_prob += pos_prob
            # tokens = F.softmax(logits, dim=-1).argmax(dim=-1)
            # texts: list[str] = self.tokenizer.batch_decode(
            #         tokens[:,-1]
            #     )
                
        return opinion1_prob/(opinion1_prob + opinion2_prob)
    
    def get_opinion_score(
        self, prompts: list[str], opinions: list[str], stop: Optional[list[str]] = None
    ):
        """Run the LLM on the given prompt and input."""
        batch_size = self.config.batch_size
        total_batches = (len(prompts) + batch_size - 1) // batch_size
        ...
        normal = lambda x: x
        space = lambda x: ' ' + x
        firstup = lambda x: x.capitalize()
        space_firstup = lambda x: ' ' + x.capitalize()
        allup = lambda x: x.upper()
        space_allup = lambda x: x.upper()

        str_trans = [normal, space, firstup, space_firstup, allup, space_allup]
        
        opinions_enc = []
        for opinion in opinions:
            opinion_enc = []
            for str_tran in str_trans:
                opinion_enc.append(self.tokenizer.encode(str_tran(opinion))[-1])
            opinions_enc.append(opinion_enc)
        ...
        for i in range(total_batches):
            ...
            for opinion_enc in opinions_enc:
                opinion_prob = 0
                for enc in opinion_enc:
                    opinion_prob += softmax[0, -1, int(enc)]
                opinions_prob.append(opinion_prob)
            
            opinions_prob = torch.FloatTensor(opinions_prob)

            # tokens = F.softmax(logits, dim=-1).argmax(dim=-1)
            # texts: list[str] = self.tokenizer.batch_decode(
            #         tokens[:,-1]
            #     )
        return opinions_prob


class HuggingFaceLLM2(LLM):
    config: ModelConfig
    model: Any = None
    tokenizer: Any = None
    device: Any = None
...
#                     print(self.tokenizer.batch_decode(tokens[:, input_ids_len:, ...].T))
    ...
    def chatbot_gen(self, prompts: str, stop: Optional[list[str]] = None,  system_prompts: str = None,):
        ...
        batched_prompts = BatchEncoding({"input_ids": input_ids}).to(self.device)
    ...
    def gen(
        ...
        for i in range(total_batches):
            ...
            pos_prob, neg_prob = 0, 0
            for j in range(len(pos_list)):
                # print(softmax[0, -1, int(pos_encoding[j])])
                # print(softmax[0, -1, int(neg_encoding[j])])
                pos_prob += softmax[0, -1, int(pos_encoding[j])]
                neg_prob += softmax[0, -1, int(neg_encoding[j])]
            ...
            # tokens = F.softmax(logits, dim=-1).argsort(dim=-1, descending=True)
            # texts: list[str] = self.tokenizer.batch_decode(
            #         tokens[:,-1]
            #     )
    ...
    def get_opinion_score(
        ...
        for i in range(total_batches):
            ...
            #         tokens[:,-1]
            #     )
            # # print(F.softmax(logits, dim=-1).max(dim=-1))
            # print(texts[0][:30])
        return opinions_prob