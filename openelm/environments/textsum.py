...
class TextSumGenotype(PromptGenotype):
    
    def __init__(
        self, textsum: str, length: int = None, formal: float = None, maturity = None,
    ):
        self.textsum = textsum
        self.length = length
    ...
    def evaluate(self, model, quality_metric, diversity_metric, ground_truth, original_text):
        ...
        if quality_metric == "ai-score":
            ...
            while quality is None:
                try:
                    quality_prompts = f"Original paragraph: {original_text}\nSummary:{self.textsum}\n On a scale from 0 to 10, rate how accurately the summary reflects the main ideas of the original text (0 = worst, 10 = best). Respond only in valid JSON with the key 'quality'. Do not include any explanation or additional text."
                    self.quality = round(float(json.loads(get_str_between("{", "}", model.gen(quality_prompts)))["quality"])/10, 4)
                except:
                    print("Retry")
    
        elif quality_metric == "logit-score":
            quality_prompts = [
                f"Original paragraph: {original_text}\nSummary:{self.textsum}\n Does this summary capture the main contexts of the original paragraph and sound good? Answer yes or no.",
                f"Original paragraph: {original_text}\nSummary:{self.textsum}\n Does this summary miss the main contexts of the original paragraph? Answer yes or no."
                ]
            self.quality = round(float(model.gen(quality_prompts).detach().cpu().numpy()), 4)

        predictions, references = [self.textsum], [ground_truth]
        self.eval_metrics = evaluate_metrics(predictions=predictions, references=references)
        
        if diversity_metric == "ai-score":
            formality_str = f"""{self.textsum}\n
            ...
            try:
                self.length = round(float(len(self.textsum.split())/len(original_text.split()))*100, 4)
                self.formality = round(float(json.loads(get_str_between("{", "}", formality_result))["formality"]), 4)
                self.maturity = round(float(json.loads(get_str_between("{", "}", maturity_result))["maturity"]), 4)
            ...
            for k, opinions_pair in enumerate(opinions_pairs):
                if k == 0:
                    prompt_list = [f"{self.textsum}\nWhat tone is this paragraph closest to from the following list: {opinions_pair}?\nAnswer: "]
                elif k == 1:
                    prompt_list = [f"{self.textsum}\nWhat audience is this paragraph more suitable for from the following list: {opinions_pair}?\nAnswer: "]
            ...
            self.length = round(len(self.textsum.split())/len(original_text.split())*100, 4)
...
class TextSumEvolution(BaseEnvironment[TextSumGenotype]):
    ...
    def set_data(self, data):
        self.ground_truth = data['highlights']
        self.original_text = data['article']
    ...
    def construct_prompt(self, textsum: Optional[TextSumGenotype] = None, nonzero=None) -> dict[str, str]:
        ...
        if nonzero is not None and nonzero.sum() != int(self.qd_config.map_grid_size[0]**len(self.config.behavior)):
            ...
            if 0 in self.config.behavior:
                max_length = self.config.behavior_space[0][1]/100.0*self.original_length
            ...
            i = 0
            if 0 in self.config.behavior:
                target_length = np.random.randint(1,self.config.behavior_space[0][1]/100.0*self.original_length)
                length_str = f'in exactly {target_length} words '
                i += 1
            else:
                target_length = None
        ...
        if self.model_config.system_prompt:
            system_prompt_str = "You are a summary chatbot. Only write the summarization text."
        else:
            system_prompt_str = None
        addi_prompt_str = "Only write the summarization text."
        ...
        if textsum is None:
            if self.config.llm_sampling:
                ...
                print("here")
            instruction_str = f"Write an abstractive summary of the above paragraph " + desc_str + addi_prompt_str
            prompt_str = f"Original paragraph:\n {self.original_text}\n{instruction_str}"
        else:
            prob = random.random()
            if prob <= self.qd_config.diversity_mutation_prob: # Diversity mutation without original context
                instruction_str = f"Translate this summarization text into another summarization text " + maturity_str + formality_str + length_str + "but do not change the abstract information." + addi_prompt_str
                prompt_str = f"Text: {textsum.textsum}.\n\n{instruction_str}"

            else: # Quality mutation with original context
                instruction_str = f"Rewrite this summary into another summary that captures more abstract information from the original text without changing features." + addi_prompt_str
                prompt_str = f"Original paragraph: {self.original_text}\nThis is a summary:\n{textsum.textsum}.\n\n{instruction_str}"
    ...
    def random(self) -> list[TextSumGenotype]:
        ...
        return [TextSumGenotype(textsum=c) for c in results]

    def mutate(self, genomes: list[TextSumGenotype], nonzero=None) -> list[TextSumGenotype]:
    ...
    def fitness(self, x: TextSumGenotype) -> float:
