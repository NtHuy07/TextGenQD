...
class QAGenotype(PromptGenotype):

    def __init__(
        self, qa: str, length: int = None, formal: float = None, maturity = None,
    ):
        self.qa = qa
        self.length = length
    ...
    def evaluate(self, model, quality_metric, diversity_metric, ground_truth, original_text, question):
        if quality_metric == "ai-score":
            quality = None
            while quality is None:
                try:
                    quality_prompts = f"Paragraph: {original_text}\nQuestion: {question}\nAnswer:{self.qa}\n On a scale from 0 to 10, rate how well (accurate and sound good) the answer response to the question given the paragraph. (0 = worst, 10 = best). Respond only in valid JSON with the key 'quality'. Do not include any explanation or additional text."
        ...
        if diversity_metric == "ai-score":
            ...
            try:
                self.length = round(float(len(self.qa.split())), 4)
...
class QAEvolution(BaseEnvironment[QAGenotype]):
    ...
    def set_data(self, data):
        self.original_text = data['document']['summary']['text']
        self.ground_truth = ""
        for answer in data['answers']:
            self.ground_truth += answer['text']
        self.question = data['question']['text']
    ...
    def construct_prompt(self, qa: Optional[QAGenotype] = None, nonzero=None) -> dict[str, str]:
        ...
        if self.model_config.system_prompt:
            system_prompt_str = "You are a chatbot who answer a question about a paragraph. Only write the answer text."
            addi_prompt_str = "Only write the answer text."
        else:
        ...
        if qa is None:
            ...
            instruction_str = f"Answer this question about the paragraph " + desc_str
            prompt_str = f"Paragraph:\n {self.original_text}\n{instruction_str} Question: {self.question}" + addi_prompt_str
        else:
            prob = random.random()
            if prob <= self.qd_config.diversity_mutation_prob: # Diversity mutation without original context
                instruction_str = f"Translate the above answer into another answer " + maturity_str + formality_str + length_str + f"but still remains the main context." + addi_prompt_str
                prompt_str = f"{qa.qa}.\n\n{instruction_str}"

            else: # Quality mutation with original context
                instruction_str = f"Rewrite this answer into another more correct answer that sound better and more natural without changing the tone and length given the paragraph." + addi_prompt_str
                prompt_str = f"Paragraph: {self.original_text}\nQuestion: {self.question}\nAnswer: {qa.qa}.\n\n{instruction_str}"
    ...
    def random(self) -> list[QAGenotype]:
        ...
        return [QAGenotype(qa=c) for c in results]

    def mutate(self, genomes: list[QAGenotype], nonzero=None) -> list[QAGenotype]:
    ...
    def fitness(self, x: QAGenotype) -> float:

        self.count += 1

        evaluation = x.evaluate(self.eval_model, quality_metric=self.quality_metric, diversity_metric=self.diversity_metric, 
                                ground_truth=self.ground_truth, original_text=self.original_text, question=self.question)
        if self.config.debug:
