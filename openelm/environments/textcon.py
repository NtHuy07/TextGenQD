...
class TextConGenotype(PromptGenotype):

    def __init__(
        self, textcon: str, length: int = None, formal: float = None, maturity = None,
    ):
    ...
    def evaluate(self, model, quality_metric, diversity_metric, ground_truth, original_text, unfinished_text):
        if quality_metric == "ai-score":
            quality = None
            while quality is None:
                try:
                    quality_prompts = f"Unfinished paragraph: {unfinished_text}\Continuation:{self.textcon}\n On a scale from 0 to 10, rate how good the continuation continues the unfinished paragraph (0 = worst, 10 = best). Respond only in valid JSON with the key 'quality'. Do not include any explanation or additional text."
            ...
            quality_prompts = [
                f"Unfinished paragraph: {unfinished_text}\Continuation:{self.textcon}\n Does this text is a good continuation for the unfinished paragraph? Answer yes or no.\n",
                ]
            self.quality = round(float(model.gen(quality_prompts).detach().cpu().numpy()), 4)
            
        predictions, references = [self.textcon], [ground_truth]
        self.eval_metrics = evaluate_metrics(predictions=predictions, references=references)

        if diversity_metric == "ai-score":
            formality_str = f"""{self.textcon}\n
            ...
            try:
                self.length = round(float(len(self.textcon.split())), 4)
...
class TextConEvolution(BaseEnvironment[TextConGenotype]):
    ...
    def set_data(self, data):
        splitted_data = data['text'].split()
        self.original_text = data['text']
        self.original_length = len(self.original_text.split())
        con_pivot = int(self.original_length * 15 / 100)
        self.ground_truth = " ".join(splitted_data[con_pivot:])
        self.unfinished_text = " ".join(splitted_data[:con_pivot])
    ...
    def construct_prompt(self, textcon: Optional[TextConGenotype] = None, nonzero=None) -> dict[str, str]:
        ...
        if self.model_config.system_prompt:
            system_prompt_str = "You are a chatbot who continue an unfinished paragraph. Only write the continuation text."
        else:
            system_prompt_str = None
        addi_prompt_str = "Only write the continuation text."
        ...
        if textcon is None:
            ...
            instruction_str = f"Write a continuation of the above unfinished paragraph " + desc_str + addi_prompt_str
            prompt_str = f"Unfinished paragraph:\n {self.unfinished_text}\n{instruction_str}\nAnswer:"
        else:
            prob = random.random()
            if prob <= self.qd_config.diversity_mutation_prob: # Diversity mutation without original context
                instruction_str = f"Translate this continuation text into another continuation text " + maturity_str + formality_str + length_str + "but do not change the abstract information." + addi_prompt_str
                prompt_str = f"{textcon.textcon}.\n\n{instruction_str}\nAnswer:"

            else: # Quality mutation with original context
                instruction_str = f"Rewrite this continuation text into another continuation text that sound more natural without changing the tone and length." + addi_prompt_str
                prompt_str = f"Unfinished paragraph: {self.unfinished_text}\nThis is a continuation:\n{textcon.textcon}.\n\n{instruction_str}\nAnswer:"
    ...
    def random(self) -> list[TextConGenotype]:
        ...
        return [TextConGenotype(textcon=c) for c in results]

    def mutate(self, genomes: list[TextConGenotype], nonzero=None) -> list[TextConGenotype]:
    ...
    def fitness(self, x: TextConGenotype) -> float:

        self.count += 1

        evaluation = x.evaluate(
            self.eval_model, quality_metric=self.quality_metric, diversity_metric=self.diversity_metric, 
            ground_truth=self.ground_truth, original_text=self.original_text, unfinished_text=self.unfinished_text
        )
        if self.config.debug:
