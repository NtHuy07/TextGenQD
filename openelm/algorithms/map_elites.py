...
class MAPElitesBase:
    ...
    def search(self, init_steps: int, total_steps: int, atol: float = 0.0) -> str:
        ...
        st = time.time()
        for n_steps in tbar:
            if n_steps < init_steps or self.genomes.empty or self.config.not_qd:
                ...
                # Mutate the elite.
                new_individuals = self.env.mutate(batch, self.nonzero.array)
            ...
            log_info = {f"Step_data_{self.data_id}": n_steps,
                        f"Data {self.data_id}/Max fitness": max_fitness, 
                        f"Data {self.data_id}/Min fitness": min_fitness,
                        f"Data {self.data_id}/Mean fitness": mean_fitness,
                        f"Data {self.data_id}/Coverage": coverage,
                        f"Data {self.data_id}/QD Score": qd_score,
                        f"Data {self.data_id}/Time": time.time() - st,
                        }
